...
describe("PermissionedStrategy", async () => {
    ...
    beforeEach(async () => {
        ...
        let strategyFactory = await ethers.getContractFactory("PermissionedStrategy")
        let ERC20Factory = await ethers.getContractFactory("TestERC20")
        vaultAsset = await ERC20Factory.deploy("Test Token", "TST")
        mockShortToken = await ERC20Factory.deploy("Mock Short Token", "3S-ETH/USD")
    ...
    describe("value", async() => {
        it("returns the sum of the vault asset on hand plus outstanding debt", async() => {
            ...
            await vaultAsset.transfer(strategy.address, ethers.utils.parseEther("2"))

            // get an account to claim 1 vaultAsset worth of debt
            await strategy.connect(accounts[1]).pullAsset(ethers.utils.parseEther("1"), vaultAsset.address)
    ...
    describe("pullAsset", async() => {
        ...
        beforeEach(async() => {
            await mockBlacklistToken.transfer(strategy.address, ethers.utils.parseEther("5"))
            await vaultAsset.transfer(strategy.address, ethers.utils.parseEther("5"))
        })

        it("only allows the pulling of whitelisted assets", async() => {
            await expect(strategy.connect(accounts[1]).pullAsset(ethers.utils.parseEther("1"), mockBlacklistToken.address)).to.be.revertedWith("ASSET_NOT_WL")
        })

        it("only allows whitelisted addresses to pull assets", async() => {
            await expect(strategy.connect(accounts[3]).pullAsset(ethers.utils.parseEther("1"), vaultAsset.address)).to.be.revertedWith("SENDER_NOT_WL")
        })

        it("updates each pullers debt and the total debt", async() => {
            let totalDebtBefore = await strategy.totalDebt(vaultAsset.address)
            let pullerDebtBefore = await strategy.debts(accounts[1].address, vaultAsset.address)

            await strategy.connect(accounts[1]).pullAsset(ethers.utils.parseEther("2"), vaultAsset.address)
            
            let totalDebtAfter = await strategy.totalDebt(vaultAsset.address)
            let pullerDebtAfter = await strategy.debts(accounts[1].address, vaultAsset.address)

            expect(totalDebtAfter.sub(totalDebtBefore).toString()).to.equal(ethers.utils.parseEther("2").toString())
            expect(pullerDebtAfter.sub(pullerDebtBefore).toString()).to.equal(ethers.utils.parseEther("2").toString())
        })
