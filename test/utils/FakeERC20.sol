...
contract FakeERC20 {
    ...
    function _setBalance(address owner, uint256 amount) public returns (uint256 newBalance) {
        ShadowedAmount memory sBal = _getSyncedBalance(owner);
        ...
        )
    {
        uint256 trueAmount = abi.decode(
            _forwardCallToImpl(abi.encodeWithSelector(IERC20.allowance.selector, owner, spender)),
            (uint256)
        );
    ...
    function _getSyncedBalance(address owner) private returns (ShadowedAmount memory sBal) {
        uint256 trueAmount = abi.decode(
            _forwardCallToImpl(abi.encodeWithSelector(IERC20.balanceOf.selector, owner)),
            (uint256)
        );
        // We only want to measure the cost of the underlying token storage lookup
        // Not including the excess overhead of our shadow lookup
    ...
    function _syncShadowedAmount(ShadowedAmount memory sAmount, uint256 trueAmount) private pure {
    ...
    function _writeSyncedBalance(address owner, ShadowedAmount memory sBal) private {
    ...
    function _getOriginalImplementationAddress() private view returns (address impl) {
        return address(uint160(address(this)) + 1);
    }

    function _forwardCallToImpl() private returns (bytes memory resultData) {
        bool success;
        (success, resultData) = _getOriginalImplementationAddress().delegatecall(msg.data);
        if (!success) {
            assembly {
    ...
    function _forwardCallToImpl(bytes memory callData) private returns (bytes memory resultData) {
        bool success;
        (success, resultData) = _getOriginalImplementationAddress().delegatecall(callData);
        if (!success) {
            assembly {
                revert(add(resultData, 32), mload(resultData))
            }
    ...
    function _transferFromInternal(
        ...
        sFromBal.shadowedAmount = _sub(sFromBal.shadowedAmount, amount, "FakeERC20/BALANCE_UNDERFLOW");
        _writeSyncedBalance(from, sFromBal);

        sToBal = _getSyncedBalance(to);
        sToBal.shadowedAmount = _add(sToBal.shadowedAmount, amount, "FakeERC20/BALANCE_OVERFLOW");
        _writeSyncedBalance(to, sToBal);

        return true;
    }

    function _updateAllowance(address from, uint256 amount) internal {
        ...
        if (from != msg.sender && sAllowance.shadowedAmount != type(uint256).max) {
            sAllowance.shadowedAmount = _sub(sAllowance.shadowedAmount, amount, "FakeERC20/ALLOWANCE_UNDERFLOW");
