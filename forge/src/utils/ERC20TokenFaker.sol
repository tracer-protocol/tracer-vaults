...
abstract contract ERC20TokenFaker {
    ...
    function fakeOutERC20(address token) internal returns (FakeERC20 fakedToken) {
        fakeOut(token, address(FAKE).code);
        fakedToken = FakeERC20(payable(token));
    }

    function unfakeERC20(address token) internal {
        unfake(token);
    }

    function fakeOut(address orig, bytes memory code) private {
    ...
    function unfake(address addr) private {
    ...
    function movedAddress(address orig) private returns (address moved) {
